...
PORTS = {"qwen36": "18201", "gemma4": "18202", "medgemma27b": "18203"}
...
def validate_configs():
    for model in MODELS:
        for stem in CONFIG_STEMS:
            path = ROOT / "configs" / "final" / f"pathvqa_{model}_{stem}.yaml"
            cfg = load_yaml(path)
            inf = cfg.get("inference", {})
            expect(cfg.get("data_root") == "data/vqa", f"{path.name}: data_root must be data/vqa")
            expect(cfg.get("datasets") == ["pathvqa"], f"{path.name}: datasets must be [pathvqa]")
            expect(cfg.get("k") == 6, f"{path.name}: k must be 6")
            expect(f":{PORTS[model]}/v1" in inf.get("base_url", ""), f"{path.name}: wrong vLLM port")
            ...
            expect(inf.get("image_max_side") is None, f"{path.name}: image_max_side must be null")
            expect("lang" not in cfg, f"{path.name}: Path-VQA config must not pass lang")
            if stem == "noicl_fixed6_random6":
                expect(cfg.get("methods") == ["zero_shot", "fixed_random_6", "random_icl"], f"{path.name}: bad noicl methods")
                expect(cfg.get("fixed_exemplars_json") == "data/vqa/pathvqa/fixed_exemplars_seed3407.json", f"{path.name}: fixed exemplar path mismatch")
    ...
    for name in ("pathvqa_extract_clip_global.yaml", "pathvqa_extract_dinov3_global.yaml"):
        cfg = load_yaml(ROOT / "configs" / "final" / name)
        expect(cfg.get("datasets") == ["pathvqa"], f"{name}: datasets must be [pathvqa]")
        expect("lang" not in cfg, f"{name}: Path-VQA config must not pass lang")


def validate_data():
    manifest = ROOT / "data" / "vqa" / "pathvqa" / "manifest.json"
    fixed = ROOT / "data" / "vqa" / "pathvqa" / "fixed_exemplars_seed3407.json"
    expect(manifest.exists(), "Path-VQA manifest missing; run scripts/prepare_pathvqa_dataset.py")
    expect(fixed.exists(), "Path-VQA fixed exemplars missing; run scripts/prepare_pathvqa_dataset.py")

    data = json.loads(manifest.read_text(encoding="utf-8"))
    splits = data.get("splits", {})
    expect(splits.get("reference", 0) > 0, "reference split is empty")
    expect(splits.get("validation", 0) > 0, "validation split is empty")
    expect(splits.get("test", 0) > 0, "test split is empty")

    dataset = get_dataset("pathvqa", str(ROOT / "data" / "vqa"), split="all")
    ...
    answer_types = {sample.question_type for sample in dataset.samples}
    expect({"OPEN", "CLOSED"}.issubset(answer_types), f"answer types missing: {answer_types}")
    ...
    parsed = OutputParser().parse_vqa('{"answer":"yes","confidence":0.9,"evidence":"visible finding"}', "smoke")
    expect(parsed.parse_success and parsed.answer == "yes", "VQA JSON parser failed")
...
def main():
    ...
    if not args.skip_data:
        validate_data()
    print("OK: Path-VQA setup is aligned")
