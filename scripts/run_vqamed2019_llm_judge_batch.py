...
def build_runs() -> list[dict]:
    ...
    for model in MODELS:
        for method in METHODS:
            run_key = f"vqamed2019_{model}_{method['key']}"
            predictions_json = (
                ROOT
                / "outputs"
                / "final"
                / f"vqamed2019_{model}_{method['dir_suffix']}"
                / "vqamed2019"
                / method["pred_dir"]
                / "predictions.json"
            )
            judge_dir = ROOT / "outputs" / "judge" / "vqamed2019_main"
            batch_dir = ROOT / "outputs" / "judge" / "vqamed2019_batch" / run_key
            runs.append(
                {
                    "run_key": run_key,
                    "model": model,
...
def main():
    ...
    parser.add_argument("--method-key", action="append", choices=[m["key"] for m in METHODS])
    ...
    parser.add_argument("--report-path", type=Path, default=ROOT / "outputs/judge/vqamed2019_batch/report.json")
